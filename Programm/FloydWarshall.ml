...
let fikseerimine(tipud) =
	...
	tekst := !tekst ^ " " ^ "Kui lahtris olev väärtus on suurem kui vastava valitud veeru ja tulba väärtuste summa, siis uuendame seda.";
	i := LahtriVaatlus;;
	...
	if !x >= (Array.length !tabel)			(* kui tabel sai läbi, lähme kas uut rida&veergu fikseerima või lõpule *)
		then (
			...
			if !fiks < Array.length !tabel - 1
				then (
					tekst := !tekst ^ " " ^ "Lähme järgmist rida ja veergu fikseerima.";
					i := Fikseerimine
				)
			else (
				i := Lopp
